...
fn dw(state: &mut State, db: Form) -> Result<Option<LabelRef>, String> {
    ...
    if !db.exps.is_empty() {
        ...
        for exp in &db.exps {
            ...
            } else if let Some(tile) = is_gameboy_tile(exp) {
                let v = tile_to_u16(tile)?;
                sec.memory.push_u16(v);
            } else {
                return Err(
                    format!("dw: immediate or tile data expected, got {:?}", exp).to_string(),
                );
            }
        }
    } else {
...
fn tile_to_u16(tile: &str) -> Result<u16, String> {
    if tile.len() != 8 {
        return Err("tile data needs exact 8 pixel definitions".to_string());
    }
    ...
    for c in tile.chars() {
        ...
        match c {
            ...
            _ => return Err("illegal tile char".to_string()),
        }
    }
    Ok(col_0 << 8 | col_1)
}
